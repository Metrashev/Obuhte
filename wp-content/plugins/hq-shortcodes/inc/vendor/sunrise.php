...
if (!class_exists('hqnrise4')) {
    ...
    class Sunrise4 {
        ...
        function __construct($args = array()) {
            ...
            if (!$args['file'])
                wp_die('hqnrise: please specify plugin __FILE__');
            if (!$args['slug'])
        ...
        public function assets() {
            // Register styles
            wp_register_style('hqnrise', plugins_url($this->config['css'], $this->config['file']) . '/sunrise.css', false, $this->config['version'], 'all');
            // Register scripts
            wp_register_script('form', plugins_url($this->config['js'], $this->config['file']) . '/form.js', array('jquery'), $this->config['version'], true);
            wp_register_script('hqnrise', plugins_url($this->config['js'], $this->config['file']) . '/sunrise.js', array('form'), $this->config['version'], true);
            // Add some l10n to JS
            wp_localize_script('hqnrise', 'sunrise', array(
        ...
        public function defaults() {
            ...
            if (get_option('hqnrise_defaults_' . $this->config['slug']))
                return;
            ...
            if (isset($this->config['options']) && is_array($this->config['options'])) {
                ...
                // Defaults is set
                update_option('hqnrise_defaults_' . $this->config['slug'], true);
            }
        }
...
if (!class_exists('hqnrise4_Views')) {
