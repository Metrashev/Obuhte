...
class HQ_Tools {
    ...
    // 	// Get data from cache
    // 	$users = ( HQ_ENABLE_CACHE ) ? get_transient( 'su/users_cache' ) : false;
    // 	// Query users
    ...
    public static function reset_users_cache() {
        if (( isset($_GET['update']) || isset($_GET['updated'])))
            if ($_GET['update'] === 'del' || $_GET['update'] === 'add' || $_GET['updated'] === '1')
                delete_transient('hq/users_cache');
    }
    ...
    public static function example() {
        ...
        if (!isset($_REQUEST['code']) || !isset($_REQUEST['id']))
            return;
        // Check for cache
        $output = get_transient('hq/examples/render/' . sanitize_key($_REQUEST['id']));
        ...
        // Cache not found
            ...
            $chunks = explode('-----', $code);
            // Show snippets
            do_action('hq/examples/preview/before');
            ...
            do_action('hq/examples/preview/after');
            $output = ob_get_contents();
            ob_end_clean();
            set_transient('hq/examples/render/' . sanitize_key($_REQUEST['id']), $output);
            echo $output;
        }
        die();
    }
    ...
    public static function reset_examples() {
        foreach ((array) HQ_Data::examples() as $example)
            foreach ((array) $example['items'] as $item)
                delete_transient('hq/examples/render/' . $item['id']);
    }
